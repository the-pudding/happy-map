<script>
  ...
  function updateCompassViewport() {
    ...
    viewportLeft = Math.max(padding, Math.min(padding + compassInnerSize, leftPercent * compassInnerSize + padding));
    const right = Math.max(padding, Math.min(padding + compassInnerSize, rightPercent * compassInnerSize + padding));
    ...
    viewportTop = Math.max(padding, Math.min(padding + compassInnerSize, topPercent * compassInnerSize + padding));
    const bottom = Math.max(padding, Math.min(padding + compassInnerSize, bottomPercent * compassInnerSize + padding));
  ...
  onMount(async () => {
    ...
    new WaterLayer({ pane: "waterPane", tileSize: 256, minZoom: 0, maxZoom: 6 }).addTo(map);
  ...
  function matchesFilters(p) {
    ...
    if (age < 20) { if (!filters.age.range1) return false; }
    else if (age >= 20 && age < 30) { if (!filters.age.range2) return false; }
    else if (age >= 30 && age < 40) { if (!filters.age.range3) return false; }
    else if (age >= 40 && age < 50) { if (!filters.age.range4) return false; }
    else if (age >= 50 && age < 60) { if (!filters.age.range5) return false; }
    else if (age >= 60 && age < 70) { if (!filters.age.range6) return false; }
    ...
    // 3. Sex
    const sex = p[5];
    if (sex === 'm') { if (!filters.sex.m) return false; }
    else if (sex === 'f') { if (!filters.sex.f) return false; }
    else { if (!filters.sex.o) return false; }
    ...
    if (marital.includes("single")) { if (!filters.marital.single) return false; }
    else if (marital.includes("married")) { if (!filters.marital.married) return false; }
    else if (marital.includes("divorced")) { if (!filters.marital.divorced) return false; }
  ...
  function renderViewport() {
    ...
    if (zoom >= 0) {
      ...
      for (let i = 0; i < allDots.length; i++) {
        ...
        const pLng = p[0] * 256;
        if (
          pLat > bounds.getSouth() && pLat < bounds.getNorth() &&
    ...
    for (const index of visibleDotIndices) {
      ...
      if (activeDotMarkers.has(index)) {
        ...
        const marker = L.marker([-(p[1] * 256), p[0] * 256], { icon: icon, zIndexOffset: 10 })
    ...
    for (const l of labelsToConsider) {
      ...
      const spansHtml = lines.map((line) => `<span style="display: block; white-space: nowrap !important; font-size: ${fontSize}px !important;">${line}</span>`).join("");
      ...
      if (activeLabelMarkers.has(labelId)) {
        ...
        const marker = L.marker([lblLat, lblLng], { icon: icon, interactive: false, zIndexOffset: zIndex });
        marker.addTo(map);
...
{#if !introShown}
  ...
  <div class="filterPanel {isFilterPanelOpen ? 'open' : ''}">
    ...
    <div class="filter-group">
      ...
      <label><input type="checkbox" bind:checked={filters.location.us} /> U.S.</label>
      <label><input type="checkbox" bind:checked={filters.location.nonUs} /> Non-U.S.</label>
    </div>
    <!-- ... include other filter groups ... -->
    <div class="filter-group">
      <div class="group-header"><div class="group-title">Age</div><div class="group-actions"><button onclick={() => toggleGroup('age', true)}>All</button><button onclick={() => toggleGroup('age', false)}>None</button></div></div>
      <label><input type="checkbox" bind:checked={filters.age.range1} /> Under 20</label>
      <label><input type="checkbox" bind:checked={filters.age.range2} /> 20 - 29</label>
      <label><input type="checkbox" bind:checked={filters.age.range3} /> 30 - 39</label>
      <label><input type="checkbox" bind:checked={filters.age.range4} /> 40 - 49</label>
      <label><input type="checkbox" bind:checked={filters.age.range5} /> 50 - 59</label>
      <label><input type="checkbox" bind:checked={filters.age.range6} /> 60 - 69</label>
      <label><input type="checkbox" bind:checked={filters.age.range7} /> 70+</label>
    </div>
    <div class="filter-group">
      <div class="group-header"><div class="group-title">Sex</div><div class="group-actions"><button onclick={() => toggleGroup('sex', true)}>All</button><button onclick={() => toggleGroup('sex', false)}>None</button></div></div>
      <label><input type="checkbox" bind:checked={filters.sex.m} /> Male</label>
      <label><input type="checkbox" bind:checked={filters.sex.f} /> Female</label>
      <label><input type="checkbox" bind:checked={filters.sex.o} /> Other</label>
    </div>
    <div class="filter-group">
      <div class="group-header"><div class="group-title">Parental Status</div><div class="group-actions"><button onclick={() => toggleGroup('parent', true)}>All</button><button onclick={() => toggleGroup('parent', false)}>None</button></div></div>
      <label><input type="checkbox" bind:checked={filters.parent.yes} /> Parent</label>
      <label><input type="checkbox" bind:checked={filters.parent.no} /> Not Parent</label>
    </div>
    <div class="filter-group">
      <div class="group-header"><div class="group-title">Marital Status</div><div class="group-actions"><button onclick={() => toggleGroup('marital', true)}>All</button><button onclick={() => toggleGroup('marital', false)}>None</button></div></div>
      <label><input type="checkbox" bind:checked={filters.marital.single} /> Single</label>
      <label><input type="checkbox" bind:checked={filters.marital.married} /> Married</label>
      <label><input type="checkbox" bind:checked={filters.marital.divorced} /> Divorced</label>
    </div>
  </div>
  ...
  <div class="infoPanel {infoShown ? 'open' : ''}">
    ...
    <h1>happy map</h1>
    <div class="byline">by <a href="https://pudding.cool/author/alvin-chang/" target="_blank">alvin chang</a></div>

    <div class="info-content">
      <p>In 2017, more than 10,000 people tracked experiences that made them happy as part of a research project. The researchers gathered more than 100,000 happy moments.</p>

      <h4>Data methodology</h4>
      <p>I initially used the <span>gemini-2.0-flash-001</span> LLM model to determine how much an agency score and time score (immediate vs. long-term), as well as a brief summary of the happy moment. These results helped me create a three-level topic taxonomy that are analgous to continents, countries, and states; there are obviously many ways to group these moment, so this is reflective of how I think about happy moments. I then prompted <span>gemini-2.0-flash-001</span> to categorize each happy moment into the three-level taxonomy. I cleaned up the results in <a href="https://openrefine.org/">Open Refine</a> to ensure there is no cross-taxonomy categorization; for example, the LLM sometimes put a moment into a state that is not within the proper parent country or continent. (This error is akin to saying that a moment belongs in New York state, and in the country of France.) From there, I used <span>gemini-2.0-flash-001</span> to give each moment a score of how well it fits into its assigned categories, since sometimes moments didn't have a perfect category in the taxonomy; what you are more likely to see on the screen on the best fits.</p>
      <h4>Map methodology</h4>
      <p>The map is initially split up into 200,000 regions using a <a href="https://en.wikipedia.org/wiki/Voronoi_diagram">voronoi</a> algorithm; it's looks like a giraffe's fur. From there, the top-level category—the "continents"—are initially given a starting atom, based on the average agency and time scores. It is then allowed to take over atoms based on the countries, states, and cities (moments) that it needs to fit within its bounds. That conquering process determined what the map looks like. This ultimately means that the moments mostly follow the x- and y-axis of time and agency, like a scatterplot. But since two things can't occupy the same space, it's not a perfect scatterplot. The moments are then placed on the map with illustrations of people. The people were drawn on paper with an ink pen and fountain pen, then digitized so they could be used as icons.</p>
      <h4>Data citation</h4>
      <p class="cite">Akari Asai, Sara Evensen, Behzad Golshan, Alon Halevy, Vivian Li, Andrei Lopatenko, Daniela Stepanov, Yoshihiko Suhara, Wang-Chiew Tan, Yinzhan Xu, "HappyDB: A Corpus of 100,000 Crowdsourced Happy Moments", LREC '18, May 2018.</p>
      <p class="links">
        <a href="https://www.researchgate.net/publication/322695424_HappyDB_A_Corpus_of_100000_Crowdsourced_Happy_Moments" target="_blank">Paper</a> |
        <a href="https://megagonlabs.github.io/HappyDB/" target="_blank">Data</a>
      </p>
    </div>
  </div>
{/if}
...
<div class="first_popup" transition:fade>
  <h1>happy map :)</h1>
  <div class="byline">by <a href="https://pudding.cool/author/alvin-chang/">alvin chang</a></div>
  <div class="introBody">
    <p>This is a map of 100,000+ happy moments from around the world. They are positioned based on the <span class="introSpan">agency</span> people had in creating them and whether they happened <span class="introSpan">immediately</span> or through <span class="introSpan">long-term</span> effort.</p>     <div class="compass">
    <div class="yaxis"></div>
    <div class="compassLabel ylabel top">Immediate</div>
    <div class="compassLabel ylabel bottom">Long-term</div>
    <div class="xaxis"></div>
    ...
    <p class="cite">Data: Akari Asai, Sara Evensen, Behzad Golshan, Alon Halevy, Vivian Li, Andrei Lopatenko, Daniela Stepanov, Yoshihiko Suhara, Wang-Chiew Tan, Yinzhan Xu, "HappyDB: A Corpus of 100,000 Crowdsourced Happy Moments", LREC '18, May 2018. | <a href="https://www.researchgate.net/publication/322695424_HappyDB_A_Corpus_of_100000_Crowdsourced_Happy_Moments">Paper</a> | <a href="https://megagonlabs.github.io/HappyDB/">Data</a></p>
  </div>
  <button id="explore" onclick={() => introToggle(false)}>Explore</button>
</div>
{/if}
...
<style>
  ...
  .first_popup h1, .first_popup .byline {
    font-size: 13px;
    line-height: 15px;
    margin: 0 0 20px 0;
    text-align: center;
  ...
  .first_popup .byline a {
    color: #eb8aff !important;
     border-bottom: 1px dotted #f263ff;
  }
  .first_popup a:hover {
    color: #68ddde;
    text-decoration: none;
  ...
  .introBody { font-size: 13px; line-height:22px; color: #dff4f5;}

  .introBody p .introSpan { color: #ffec70; }
  .cite {
    font-size: 11px;
    line-height: 14px;
    margin-top: 10px;
  ...
  .wrapper {
    ...
    background-color: #1e3d54; z-index: 1;
    overflow: hidden;
  }

  .map { width: 100%; height: 100vh; background-color: #003a4a; }
  ...
  :global(.water-tile) {
    ...
    background-size: 64px 32px; background-repeat: repeat;
  }

  /* COMPASS */
  .compass {
    position: absolute; left: 10px; bottom: max(10px, env(safe-area-inset-bottom, 10px));
    width: clamp(130px, 18vw, 180px); height: clamp(130px, 18vw, 180px);
    background: #002436; z-index: 999; border: 2px solid #264a5c;
    border-radius: 4px; overflow: hidden; font-family: var(--sans);
  }
  .viewport-box { position: absolute; background: rgba(158, 255, 220, 0.4); pointer-events: none; z-index: 50; border-radius: 1px; }
  .yaxis { position: absolute; left: 50%; top: 11%; height: 78%; border-left: 1px solid rgb(232, 249, 255); z-index: 60; }
  .xaxis { position: absolute; top: 50%; left: 11%; width: 78%; border-top: 1px solid rgb(232, 249, 255); z-index: 60; }
  .compassLabel { position: absolute; font-size: clamp(10px, 1.2vw, 12px); line-height: 1; font-weight: 300; color: rgb(232, 249, 255); z-index: 70; }
  ...
  /* ELEMENTS */
  :global(.map-label), :global(.interactive-dot) { opacity: 1; }
  :global(.map-element-visible) { opacity: 1 !important; }
  :global(.interactive-dot) { cursor: pointer; }
  :global(.map-label) {
  ...
  :global(.label-l3 span) {
    font-family: var(--sans); font-weight: 400; color: #ffffff; text-transform: none;
    letter-spacing: 0px;
  ...
  .filterButton {
    ...
    background: #002436; border: 1px solid #264a5c;
    color: white; padding: 8px 12px; cursor: pointer;
    text-shadow: 1px 1px 0 #000; font-family: var(--sans);
    z-index: 9; border-radius: 4px;
    font-size: 15px;
  }
  #showinfo {
  ...
  .filterPanel h4 {
    ...
    border-bottom: 1px solid #264a5c; padding-bottom: 6px;
  }
  ...
  .group-title {
    ...
    color: #9effdc; font-weight: bold;
  }
  .group-actions button {
    background: none; border: none; color: #5aa6cf;
    font-size: 10px; cursor: pointer; padding: 0 4px;
    text-transform: uppercase;
  }
  .group-actions button:hover { color: white; text-decoration: underline; }
  ...
  .filterPanel label:hover { color: white; }
  ...
  .infoPanel a { color: #57bff2; text-decoration: none; border-bottom: 1px dotted #57bff2; }
  .infoPanel a:hover { color: #68ddde; border-bottom: 1px solid #4da7a8; }
  ...
  .infoPanel p span { color: #ffec70; }

  .infoPanel .cite { font-size: 12px; color: #8db1b3; font-style: italic; }
  .infoPanel .links { font-size: 12px; }
  ...
  .static-compass .compassLabel { font-size: 10px; }
